...
class MealPrepEntry(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    meal_prep_id = Column(UUID(as_uuid=True), ForeignKey("meal_preps.id"), nullable=False)
...
class MealPrepItem(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    meal_prep_entry_id = Column(UUID(as_uuid=True), ForeignKey("meal_prep_entries.id"), nullable=False)
